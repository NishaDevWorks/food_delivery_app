...
export function MobileShell({ children, hideNav = false, }) {
    ...
    return (<div className="min-h-screen w-full bg-gradient-to-br from-violet-100 via-pink-50 to-sky-100 flex items-center justify-center p-0 sm:p-6">
...
function NavItem({ to, icon: Icon, label, active, badge, }) {
    return (<Link to={to} className="relative flex flex-col items-center justify-center gap-1 flex-1 py-2">
      <div className={`relative p-2 rounded-2xl transition-all ${active
            ? "bg-gradient-to-br from-violet-400 to-pink-400 text-white shadow-lg shadow-pink-200"
            : "text-slate-400"}`}>
        <Icon className="w-5 h-5"/>
        {badge ? (<span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
            {badge}
          </span>) : null}
      </div>
...
function BottomNav() {
    ...
    const { count } = useCart();
    return (<nav className="absolute bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200/60 flex px-2 pb-[env(safe-area-inset-bottom)]">
