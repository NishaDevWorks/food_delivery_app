...
import { fetchMyRestaurants, isOwner } from "@/lib/owner-api";
import { supabase } from "@/integrations/supabase/client";
const OWNER_KEY = "quickbite_owner_active";
export function useActiveRestaurant() {
    const [rid, setRidState] = useState(null);
    useEffect(() => {
        try {
            setRidState(localStorage.getItem(OWNER_KEY));
        }
        catch { }
    }, []);
    const setRid = (id) => {
...
export function OwnerShell({ children }) {
    ...
    if (loading) {
        return (<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-100 via-pink-50 to-sky-100">
        <div className="w-8 h-8 border-2 border-violet-400 border-t-transparent rounded-full animate-spin"/>
      </div>);
    }
    // No owner role or no restaurants → access request state
    if (!owner || mine.length === 0) {
        return (<div className="min-h-screen bg-gradient-to-br from-violet-100 via-pink-50 to-sky-100 p-6">
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-violet-50">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-xl border-b border-slate-200/60">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0 relative">
            <button onClick={() => setSwitchOpen((v) => !v)} className="flex items-center gap-2 px-2 py-1 rounded-xl hover:bg-slate-100 max-w-full">
            ...
            {switchOpen && mine.length > 1 && (<div className="absolute top-full mt-1 left-0 bg-white rounded-2xl shadow-xl border border-slate-200 p-1 w-64 z-50">
                {mine.map((r) => (<button key={r.id} onClick={() => { setRid(r.id); setSwitchOpen(false); }} className={`w-full flex items-center gap-2 p-2 rounded-xl text-left ${r.id === active.id ? "bg-violet-50" : "hover:bg-slate-50"}`}>
        ...
        <nav className="max-w-6xl mx-auto px-2 flex gap-1 overflow-x-auto no-scrollbar">
          {tabs.map((t) => {
            ...
            return (<Link key={t.to} to={t.to} className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold border-b-2 whitespace-nowrap ${active ? "border-violet-500 text-violet-600" : "border-transparent text-slate-500 hover:text-slate-800"}`}>
