...
export type RzpMethod = "upi" | "card" | "netbanking" | "wallet" | "paylater";

const TABS: { id: RzpMethod | "recommended"; label: string; Icon: any; badges: string[] }[] = [
...
type Props = {
  ...
  merchantName?: string;
  onClose: () => void;
  onSuccess: (method: RzpMethod, meta: Record<string, string>) => void;
};

export function RazorpayCheckout({ open, amount, phone, merchantName = "QuickBite", onClose, onSuccess }: Props) {
  const [tab, setTab] = useState<RzpMethod | "recommended">("recommended");
  ...
  const activeMethod: RzpMethod = tab === "recommended" ? "upi" : tab;
  ...
  function validate(): string | null {
    if (activeMethod === "upi") {
      ...
      if (!/^\d{16}$/.test(n)) return "Card number must be 16 digits";
      if (!/^(0[1-9]|1[0-2])\/\d{2}$/.test(cardExp)) return "Expiry must be MM/YY";
  ...
  function pay() {
    ...
    if (activeMethod === "upi") meta.upiId = upiId;
    if (activeMethod === "card") meta.card = "•••• " + cardNum.replace(/\s/g, "").slice(-4);
    if (activeMethod === "netbanking") meta.bank = bank;
    if (activeMethod === "wallet") meta.wallet = wallet;
    if (activeMethod === "paylater") meta.provider = paylater;
    setTimeout(() => {
      setProcessing(false);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-3 animate-in fade-in">
      <div className="w-full max-w-[860px] max-h-[92vh] rounded-2xl overflow-hidden shadow-2xl bg-white flex flex-col md:flex-row relative">
          ...
          {TABS.map(({ id, label, Icon }) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 md:w-full flex items-center gap-2.5 px-4 py-3 text-left text-sm border-l-[3px] transition ${
                  active
        ...
        <section className="flex-1 p-5 overflow-y-auto min-h-[360px]">
          ...
          <div className="mt-4 space-y-3">
            {activeMethod === "upi" && (
              <>
                ...
                <input
                  ...
                  className="w-full h-11 px-3 rounded-lg border border-slate-300 text-sm focus:outline-none focus:border-[#0b3fb8] focus:ring-2 focus:ring-[#0b3fb8]/20"
                />
                <div className="flex gap-2 pt-1">
                  {["success@upi", "test@ybl", "demo@okhdfc"].map((s) => (
                    <button key={s} onClick={() => setUpiId(s)} className="text-[11px] px-2 py-1 rounded bg-slate-100 text-slate-600 hover:bg-slate-200">
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                {BANKS.map((b) => (
                  <button
                    key={b}
                    onClick={() => setBank(b)}
                    className={`w-full flex items-center justify-between p-3 rounded-lg border text-sm text-left transition ${
                      bank === b ? "border-[#0b3fb8] bg-[#0b3fb8]/5 text-[#0b3fb8] font-semibold" : "border-slate-200 hover:bg-slate-50 text-slate-700"
                    }`}
                  >
              ...
              <>
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-11 px-3 rounded-lg border border-slate-300 text-sm bg-white focus:outline-none focus:border-[#0b3fb8]"
                  >
            ...
            {activeMethod === "paylater" && (
              <div className="space-y-2">
                {PAYLATERS.map((p) => (
                  <button
                    key={p}
                    onClick={() => setPaylater(p)}
                    className={`w-full flex items-center justify-between p-3 rounded-lg border text-sm text-left transition ${
                      paylater === p ? "border-[#0b3fb8] bg-[#0b3fb8]/5 text-[#0b3fb8] font-semibold" : "border-slate-200 hover:bg-slate-50 text-slate-700"
                    }`}
                  >
            ...
            {error && (
              <p className="text-xs font-semibold text-rose-600 bg-rose-50 border border-rose-200 rounded-lg p-2">{error}</p>
            )}

            <button
              onClick={pay}
              disabled={processing}
              className="w-full h-12 rounded-lg bg-slate-900 hover:bg-black text-white font-bold text-sm mt-2 disabled:opacity-70 flex items-center justify-center gap-2"
            >
              {processing ? (
                <>
