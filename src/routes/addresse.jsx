    ...
    return (<MobileShell>
      <div className="px-5 pt-6 pb-8">
        ...
        <div className="mt-5 space-y-2">
          ...
          {list.map((a) => (<div key={a.id} className="flex items-center gap-3 bg-white/90 rounded-2xl p-3 shadow-sm">
        ...
        <form onSubmit={add} className="mt-6 bg-white/90 rounded-3xl p-4 shadow-sm space-y-3">
          ...
          <div className="flex gap-2">
            {["home", "work", "other"].map((t) => (<button type="button" key={t} onClick={() => setType(t)} className={`flex-1 h-10 rounded-xl text-xs font-semibold capitalize ${type === t ? "bg-gradient-to-r from-violet-500 to-pink-500 text-white" : "bg-slate-50 text-slate-600 border border-slate-200"}`}>
                {t}
              </button>))}
          </div>
