...
import { findCoupon, fetchAvailableCoupons, bumpCouponUsage } from "@/lib/coupons";
import { addOrder, saveOrderToCloud } from "@/lib/orders";
...
function CartPage() {
    ...
    async function applyCoupon(input) {
        ...
        if (result.discount === 0 && !result.freeDelivery)
            return toast.error("Coupon not applicable");
    ...
    async function finalizeOrder(paymentLabel, isCod, rzp) {
        ...
        if (applied?.code)
            bumpCouponUsage(applied.code).catch(() => { });
        clear();
        setPaying(false);
    ...
    function onMockSuccess(method, meta) {
        setMockOpen(false);
        const label = method === "upi" ? `UPI (${meta.upiId ?? ""})` :
    ...
    return (<MobileShell>
      <div className="px-5 pt-8">
        ...
        {items.length === 0 ? (<div className="mt-20 flex flex-col items-center text-center">
            ...
            <Link to="/home" className="mt-6 px-6 h-12 rounded-2xl bg-gradient-to-r from-violet-500 to-pink-500 text-white font-semibold flex items-center shadow-md shadow-pink-200">
              Browse restaurants
            </Link>
          </div>) : (<>
            <div className="mt-5 space-y-3">
              {items.map((i) => (<div key={i.id} className="bg-white/90 rounded-2xl p-3 shadow-sm flex items-center gap-3">
                  <div className="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 shrink-0">
                    {i.image ? (<img src={i.image} alt={i.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center text-2xl">{i.emoji}</div>)}
                  </div>
            ...
            <div className="mt-5 bg-white/90 rounded-2xl p-4 shadow-sm">
              ...
              {applied ? (<div className="mt-2 flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-xl p-2">
                  <div>
                  ...
                  <div className="mt-2 flex gap-2">
                    <input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="Enter code" className="flex-1 h-10 px-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300"/>
                    <button onClick={() => applyCoupon()} className="px-4 h-10 rounded-xl bg-violet-500 text-white text-sm font-semibold">
                      Apply
                    </button>
                  </div>
                  <div className="mt-3 space-y-1">
                    {available.map((c) => (<button key={c.code} onClick={() => applyCoupon(c.code)} className="w-full flex items-center justify-between text-left p-2 rounded-lg hover:bg-violet-50">
                        <div>
            ...
            <button onClick={payOnline} disabled={paying} className="mt-5 w-full h-14 rounded-2xl bg-gradient-to-r from-violet-500 to-pink-500 text-white font-bold shadow-lg shadow-pink-200 active:scale-[0.98] transition disabled:opacity-60 flex items-center justify-center gap-2">
              {paying ? (<>
            ...
            <button onClick={payCod} disabled={paying} className="mt-2 w-full h-12 rounded-2xl bg-white border-2 border-slate-200 text-slate-800 font-semibold flex items-center justify-center gap-2 active:scale-[0.98] transition disabled:opacity-60">
              <Banknote className="w-4 h-4 text-emerald-600"/>
              Cash on Delivery
            </button>
          </>)}
      </div>
      <RazorpayCheckout open={mockOpen} amount={grand} onClose={() => setMockOpen(false)} onSuccess={onMockSuccess}/>
    </MobileShell>);
}
function Row({ label, value, bold, accent }) {
    return (<div className={`flex justify-between ${bold ? "font-bold text-slate-900" : accent ? "text-emerald-600 font-semibold" : "text-slate-600"}`}>
      <span>{label}</span>
      <span>{value}</span>
    </div>);
}
