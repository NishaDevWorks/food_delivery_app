...
function HomePage() {
    ...
    const trending = useMemo(() => restaurants
        .flatMap((r) => r.dishes.map((d) => ({ ...d, rid: r.id, rname: r.name })))
        .sort((a, b) => b.price - a.price)
        .slice(0, 8), []);
    const cheapest = (r) => Math.min(...r.dishes.map((d) => d.price));
    ...
    const activeFilterCount = (dietFilter !== "all" ? 1 : 0) + (minRating > 0 ? 1 : 0) + (maxPrice < 500 ? 1 : 0);
      ...
      <div className="px-5 pt-8">
        <div className="flex items-center justify-between">
          <button onClick={turnOnLocation} className="min-w-0 flex-1 text-left pr-3">
            ...
            {locStatus !== "ready" && locStatus !== "detecting" && (<span className="text-[11px] text-violet-600 font-semibold">Tap to turn on location</span>)}
          </button>
          {me.avatar ? (<img src={me.avatar} alt={me.name || "Your profile"} referrerPolicy="no-referrer" className="w-11 h-11 rounded-full object-cover shrink-0 shadow"/>) : (<div className="w-11 h-11 rounded-full bg-gradient-to-br from-violet-300 to-pink-300 flex items-center justify-center text-white font-bold shrink-0">
        ...
        <div className="mt-5 flex gap-2">
          <div className="relative flex-1">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search restaurants or dishes" className="w-full h-12 pl-11 pr-4 rounded-2xl bg-white/80 border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300"/>
          </div>
          <button onClick={() => setShowFilters((s) => !s)} className={`relative w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${showFilters || activeFilterCount > 0
            ? "bg-gradient-to-br from-violet-500 to-pink-500 text-white shadow-md shadow-pink-200"
            : "bg-white/80 border border-slate-200 text-slate-600"}`} aria-label="Filters">
            <SlidersHorizontal className="w-5 h-5"/>
            {activeFilterCount > 0 && (<span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                {activeFilterCount}
              </span>)}
          </button>
        </div>
        ...
        <div className="mt-5">
          ...
          <div className="mt-3 flex gap-3 overflow-x-auto pb-2 -mx-5 px-5 snap-x">
            {trending.map((d) => (<Link key={`${d.rid}-${d.id}`} to="/restaurant/$id" params={{ id: d.rid }} className="snap-start shrink-0 w-36 rounded-2xl overflow-hidden bg-white/90 shadow-sm active:scale-[0.98] transition">
                <div className="relative h-24">
                  ...
                  <span className={`absolute top-1 right-1 w-3.5 h-3.5 rounded-sm border-2 flex items-center justify-center ${d.veg ? "border-emerald-500" : "border-rose-500"} bg-white`}>
        ...
        {showFilters && (<div className="mt-3 bg-white/90 rounded-2xl p-4 shadow-sm space-y-4 animate-fade-in">
            <div className="flex items-center justify-between">
            ...
            ].map((o) => (<button key={o.k} onClick={() => setDietFilter(o.k)} className={`flex-1 h-9 rounded-xl text-xs font-semibold border ${dietFilter === o.k ? "bg-violet-500 text-white border-violet-500" : "bg-white text-slate-600 border-slate-200"}`}>
                    {o.label}
                  </button>))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[0, 4, 4.5].map((r) => (<button key={r} onClick={() => setMinRating(r)} className={`flex-1 h-9 rounded-xl text-xs font-semibold border ${minRating === r ? "bg-violet-500 text-white border-violet-500" : "bg-white text-slate-600 border-slate-200"}`}>
                    {r === 0 ? "Any" : `${r}+ ★`}
                  </button>))}
              </div>
            </div>
            ...
            <div>
              ...
              <input type="range" min={50} max={500} step={10} value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="w-full accent-violet-500"/>
            </div>
            ...
            ].map((o) => (<button key={o.k} onClick={() => setSort(o.k)} className={`flex-1 h-9 rounded-xl text-xs font-semibold border ${sort === o.k ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-200"}`}>
                    {o.label}
                  </button>))}
              </div>
            </div>
        ...
        <div className="mt-5 flex gap-2 overflow-x-auto pb-2 -mx-5 px-5">
          {categories.map((c) => (<button key={c.label} onClick={() => setCat(c.label)} className={`shrink-0 px-4 h-10 rounded-full text-sm font-medium flex items-center gap-1.5 transition ${cat === c.label
        ...
        {filtered.length === 0 ? (<p className="mt-4 text-sm text-slate-500 text-center py-8">
            ...
            {filtered.map((r) => {
                ...
                return (<div key={r.id} className="relative">
                  <Link to="/restaurant/$id" params={{ id: r.id }} className="flex gap-3 rounded-3xl bg-white/90 p-3 shadow-sm active:scale-[0.99] transition">
                    <div className={`relative w-24 h-24 shrink-0 rounded-2xl bg-gradient-to-br ${r.gradient} overflow-hidden`}>
                      <img src={r.image} alt={r.name} loading="lazy" className={`w-full h-full object-cover ${closed ? "grayscale opacity-70" : ""}`}/>
                      {closed && (<div className="absolute inset-0 flex items-center justify-center bg-black/40">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-[11px] text-slate-500">
                        ...
                        {r.vegOnly && (<span className="ml-2 text-[9px] font-bold text-emerald-600 border border-emerald-300 rounded px-1">
                            PURE VEG
                          </span>)}
                      </p>
                    </div>
                  ...
                  <button onClick={(e) => {
                    ...
                    }} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/90 backdrop-blur flex items-center justify-center shadow" aria-label="Favorite">
