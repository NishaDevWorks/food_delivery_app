...
function LoginPage() {
    ...
    function requestLocation(silent = false) {
        ...
        navigator.geolocation.getCurrentPosition((pos) => {
            try {
                localStorage.setItem("quickbite_location", JSON.stringify({ lat: pos.coords.latitude, lng: pos.coords.longitude }));
            }
            catch { }
    ...
    return (<div className="min-h-screen w-full bg-gradient-to-br from-violet-200 via-pink-100 to-sky-200 flex items-center justify-center p-0 sm:p-6">
      <div className="relative w-full sm:max-w-[420px] sm:rounded-[36px] sm:shadow-2xl bg-white/70 backdrop-blur-2xl min-h-screen sm:min-h-[760px] overflow-hidden flex flex-col p-6 pt-12">
        ...
        <div className="mt-6 bg-white/80 rounded-2xl p-1 flex">
          <button onClick={() => setTab("signin")} className={`flex-1 h-10 rounded-xl text-sm font-semibold transition ${tab === "signin" ? "bg-gradient-to-r from-violet-500 to-pink-500 text-white shadow" : "text-slate-600"}`}>
            Sign in
          </button>
          <button onClick={() => setTab("signup")} className={`flex-1 h-10 rounded-xl text-sm font-semibold transition ${tab === "signup" ? "bg-gradient-to-r from-violet-500 to-pink-500 text-white shadow" : "text-slate-600"}`}>
            Sign up
          </button>
        </div>
        ...
        <div className="mt-4 space-y-3">
          {tab === "signup" && (<>
              ...
              <div>
                <Field icon={Phone} placeholder="Phone number (10 digits)" type="tel" value={phone} onChange={(v) => setPhone(v.replace(/[^\d+\s-]/g, "").slice(0, 15))}/>
                {phone.trim() && phoneError && (<p className="mt-1 ml-1 text-[11px] font-medium text-rose-600">{phoneError}</p>)}
              </div>
            </>)}
          ...
          <button onClick={submitEmail} disabled={loading} className="w-full h-12 rounded-2xl bg-gradient-to-r from-violet-500 to-pink-500 text-white font-semibold shadow-lg shadow-pink-200 active:scale-[0.98] transition disabled:opacity-60">
        ...
        <div className="space-y-3">
          <button onClick={continueWithGoogle} disabled={loading} className="w-full h-12 rounded-2xl bg-white text-slate-800 font-semibold shadow border border-slate-200 flex items-center justify-center gap-3 active:scale-[0.98] transition disabled:opacity-60">
            <GoogleIcon />
            Continue with Google
          </button>
          <button onClick={() => requestLocation(false)} className={`w-full h-11 rounded-2xl font-medium text-sm flex items-center justify-center gap-2 transition ${locStatus === "granted"
...
function Field({ icon: Icon, placeholder, value, onChange, type = "text", }) {
    return (<div className="relative">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full h-12 pl-11 pr-4 rounded-2xl bg-white/90 border border-slate-200 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-300"/>
    </div>);
}
function GoogleIcon() {
