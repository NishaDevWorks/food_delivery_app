...
import { fetchNotifications, markAllRead, markRead, deleteNotification, type NotifRow } from "@/lib/notifications-api";
...
function NotificationsPage() {
  ...
  const prefRows: { k: keyof Prefs; label: string; desc: string }[] = [
    ...
    <MobileShell>
      <div className="px-5 pt-6 pb-8">
        <div className="flex items-center gap-3">
          ...
          {unread > 0 && (
            <button onClick={async () => { await markAllRead(); load(); }} className="text-xs font-semibold text-violet-600">
              Mark all read
            </button>
          )}
        </div>
            ...
            <div className="space-y-2">
              {items.map((n) => (
                <div
                  key={n.id}
                  onClick={() => openItem(n)}
                  className={`bg-white/90 rounded-2xl p-3 shadow-sm flex items-start gap-3 cursor-pointer ${!n.read ? "border-l-4 border-violet-500" : ""}`}
                >
                  <IconFor type={n.type} />
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteNotification(n.id).then(load); }}
...
function IconFor({ type }: { type: NotifRow["type"] }) {
  const map = {
  ...
  return (
    <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${m.c}`}>
