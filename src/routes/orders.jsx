...
import { loadOrders, fetchCloudOrders } from "@/lib/orders";
...
function OrdersPage() {
    ...
    return (<MobileShell>
      <div className="px-5 pt-8">
        ...
        {orders.length === 0 && !loading ? (<div className="mt-20 flex flex-col items-center text-center">
            ...
            {orders.map((o) => (<div key={o.id} className="bg-white/90 rounded-2xl p-4 shadow-sm">
                ...
                <div className="mt-3 flex gap-2 overflow-x-auto">
                  {o.items.slice(0, 4).map((it) => (<div key={it.id} className="shrink-0 w-12 h-12 rounded-xl overflow-hidden bg-slate-100">
                      {it.image ? (<img src={it.image} alt={it.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center text-lg">{it.emoji}</div>)}
                    </div>))}
                </div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Link to="/invoice/$id" params={{ id: o.id }} className="text-xs font-semibold text-slate-600 flex items-center gap-1">
                      <FileText className="w-3 h-3"/> Invoice
                    </Link>
                    <button className="text-xs font-semibold text-violet-600 flex items-center gap-1" onClick={() => alert("Reorder coming soon")}>
