...
function AnalyticsPage() {
  ...
  const days = useMemo(() => {
    const arr: { d: string; revenue: number; orders: number }[] = [];
    ...
    for (const o of orders) {
      const day = new Date(o.placed_at); day.setHours(0, 0, 0, 0);
      const idx = arr.findIndex((a) => a.d === day.toISOString().slice(5, 10));
      if (idx >= 0) { arr[idx].revenue += Number(o.total); arr[idx].orders += 1; }
    }
    return arr;
  }, [orders, range]);
  ...
  const bestItems = useMemo(() => {
    ...
    for (const o of orders) for (const it of o.items || []) {
      ...
      c.qty += it.qty; c.revenue += it.price * it.qty; map.set(it.name, c);
    }
  ...
  const peakHours = useMemo(() => {
    ...
    for (const o of orders) hrs[new Date(o.placed_at).getHours()] += 1;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex gap-1 bg-white rounded-2xl p-1 shadow-sm">
          {[7, 30, 90].map((r) => (
            <button key={r} onClick={() => setRange(r as Range)} className={`px-3 py-1.5 rounded-xl text-xs font-bold ${range === r ? "bg-gradient-to-r from-violet-500 to-pink-500 text-white" : "text-slate-500"}`}>
              {r}d
            </button>
          ))}
        </div>
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm">
        ...
        <div className="mt-4 flex items-end gap-1 h-40">
          {days.map((d, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1" title={`${d.d}: ₹${d.revenue}`}>
              <div
                className="w-full rounded-t bg-gradient-to-t from-violet-500 to-pink-400"
                style={{ height: `${(d.revenue / maxRev) * 100}%`, minHeight: d.revenue > 0 ? "2px" : "0" }}
              />
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm">
        ...
        <div className="mt-3 grid grid-cols-24 gap-0.5" style={{ gridTemplateColumns: "repeat(24, 1fr)" }}>
          {peakHours.hrs.map((v, i) => (
            <div key={i} title={`${i}:00 · ${v} orders`} className="h-8 rounded" style={{
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm">
        ...
        {bestItems.length === 0 ? (
          <p className="mt-3 text-xs text-slate-400">No sales in this range.</p>
        ) : (
