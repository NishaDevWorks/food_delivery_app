...
import { fetchOwnerCoupons, createCoupon, updateCoupon, deleteCoupon } from "@/lib/coupons";
...
function CouponsPage() {
    ...
    async function load() { if (rid)
        setRows(await fetchOwnerCoupons(rid)); }
    ...
    return (<div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm((v) => !v)} className="px-4 h-10 rounded-xl bg-gradient-to-r from-violet-500 to-pink-500 text-white text-xs font-bold flex items-center gap-1">
          <Plus className="w-3.5 h-3.5"/> New coupon
        </button>
      </div>

      {showForm && (<div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            {form.type !== "free_delivery" && (<Field label={form.type === "percent" ? "Percent (%)" : "Amount (₹)"}>
            ...
            {form.type === "percent" && (<Field label="Max discount (₹)">
      ...
      {rows.length === 0 ? (<div className="text-center py-16 text-slate-400 text-sm flex flex-col items-center">
          <Tag className="w-8 h-8 mb-2"/> No coupons yet.
        </div>) : (<div className="space-y-2">
          {rows.map((c) => (<div key={c.id} className="bg-white rounded-2xl p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex gap-3 text-[11px] text-slate-500 flex-wrap">
                    ...
                    {c.expires_at && (<span className="flex items-center gap-0.5"><Calendar className="w-3 h-3"/> {new Date(c.expires_at).toLocaleDateString()}</span>)}
                  </div>
                </div>
...
function Field({ label, children, full }) {
    return (<label className={`block ${full ? "col-span-2" : ""}`}>
