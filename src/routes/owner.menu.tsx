...
import { addMenuItem, deleteMenuItem, fetchMenuItems, updateMenuItem, type MenuItem } from "@/lib/owner-api";
...
function MenuPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!rid || !name || !price) return toast.error("Name and price required");
    setSubmitting(true);
    try {
      await addMenuItem({
    ...
    <div className="grid md:grid-cols-[380px_1fr] gap-4">
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-3xl p-10 text-center text-sm text-slate-400">
            No menu items yet. Add your first dish on the left.
          </div>
        ) : (
          CATEGORIES.filter((c) => grouped[c]?.length).map((cat) => (
            <div key={cat} className="bg-white rounded-3xl p-5 shadow-sm">
              <h3 className="text-sm font-black text-slate-900 mb-3">{cat}</h3>
              <div className="space-y-2">
                {grouped[cat].map((it) => (
                  <div key={it.id} className="flex items-center gap-3 p-2 rounded-2xl hover:bg-slate-50">
                    ...
                    <button
                      ...
                      className={`text-[10px] font-bold px-2 py-1 rounded-full ${it.in_stock ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-500"}`}
                    >
