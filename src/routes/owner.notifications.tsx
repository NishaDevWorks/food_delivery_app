  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        {unread > 0 && (
          <button onClick={async () => { await markAllRead(); load(); }} className="text-xs font-semibold text-violet-600 px-3 h-9 rounded-xl bg-violet-50">
            Mark all read
          </button>
        )}
      </div>
        ...
        <div className="space-y-2">
          {items.map((n) => (
            <div
              key={n.id}
              onClick={() => openItem(n)}
              className={`bg-white rounded-2xl p-3 shadow-sm flex items-start gap-3 cursor-pointer ${!n.read ? "border-l-4 border-violet-500" : ""}`}
            >
              <IconFor type={n.type} />
