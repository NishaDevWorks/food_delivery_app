...
import { fetchOwnerOrders, updateOrderStatus, type OwnerOrder } from "@/lib/owner-api";
...
const FLOW = ["preparing", "out_for_delivery", "delivered"] as const;
...
function OwnerOrdersPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const row = payload.new as any as OwnerOrder;
  ...
  const shown = useMemo(() => {
    ...
    if (tab === "completed") return orders.filter((o) => ["delivered", "cancelled"].includes(o.status));
    return orders.filter((o) => !["delivered", "cancelled"].includes(o.status));
  }, [orders, tab]);

  async function advance(o: OwnerOrder) {
    const idx = FLOW.indexOf(o.status as any);
  ...
  async function cancel(o: OwnerOrder) {
      ...
      <div className="flex gap-1 bg-white rounded-2xl p-1 w-fit shadow-sm">
        {(["active", "completed", "all"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-1.5 rounded-xl text-xs font-bold capitalize ${tab === t ? "bg-gradient-to-r from-violet-500 to-pink-500 text-white" : "text-slate-500"}`}
          >
            {t}
          </button>
        ))}
            ...
            <div key={o.id} className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {(o.items || []).map((it: any, i: number) => (
                  <span key={i} className="text-[11px] bg-slate-100 rounded-full px-2 py-0.5 text-slate-700">
                    {it.qty}× {it.name}
                  </span>
                ))}
              </div>
              ...
              {!["delivered", "cancelled"].includes(o.status) && (
                <div className="mt-3 flex gap-2">
                  <button
                    disabled={busy === o.id}
                    onClick={() => advance(o)}
                    className="flex-1 h-10 rounded-xl bg-gradient-to-r from-violet-500 to-pink-500 text-white text-xs font-bold flex items-center justify-center gap-1 disabled:opacity-60"
                  >
                  ...
                  <button
                    disabled={busy === o.id}
                    onClick={() => cancel(o)}
                    className="h-10 px-3 rounded-xl bg-rose-50 text-rose-600 text-xs font-bold disabled:opacity-60"
                  >
                    Cancel
                  </button>
                </div>
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${m.c}`}>
