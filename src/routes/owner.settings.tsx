...
import { fetchSettings, upsertSettings, type RestaurantSettings } from "@/lib/owner-api";
...
function SettingsPage() {
  ...
  const [s, setS] = useState<Partial<RestaurantSettings>>({});
  ...
  return (
    <div className="max-w-xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm flex items-center justify-between">
        ...
        <button
          onClick={() => setS({ ...s, is_open: !s.is_open })}
          className={`w-14 h-8 rounded-full p-1 transition ${s.is_open ? "bg-emerald-500" : "bg-slate-300"}`}
        >
