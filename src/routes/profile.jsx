    ...
    return (<MobileShell>
      <div className="px-5 pt-8">
        ...
        <div className="mt-5 bg-white/90 rounded-3xl p-5 shadow-sm flex items-center gap-4">
          {profile.avatar ? (<img src={profile.avatar} alt={profile.name} className="w-16 h-16 rounded-2xl object-cover shadow" referrerPolicy="no-referrer"/>) : (<div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-400 to-pink-400 flex items-center justify-center text-white text-2xl font-bold">
              {initial}
            </div>)}
        ...
        <div className="mt-5 bg-white/90 rounded-3xl shadow-sm divide-y divide-slate-100 overflow-hidden">
          {items.map(({ icon: Icon, label, to }) => (<Link key={label} to={to} className="w-full flex items-center gap-3 p-4 text-left">
        ...
        <button onClick={signOut} className="mt-5 w-full h-12 rounded-2xl bg-white border border-rose-200 text-rose-600 font-semibold flex items-center justify-center gap-2">
