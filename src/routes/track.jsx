...
function TrackPage() {
    ...
    useEffect(() => {
        ...
        if (!isUuid)
            return; // local-only order (not synced to cloud)
        const channel = supabase
            ...
            if (remoteStatus === "delivered")
                setStepIdx(3);
            else if (remoteStatus === "out_for_delivery")
                setStepIdx((s) => Math.max(s, 2));
            const merged = { ...activeOrder, status: remoteStatus, paymentStatus: row.payment_status };
            setActiveOrder(merged);
            try {
                localStorage.setItem("quickbite_active_order", JSON.stringify(merged));
            }
            catch { }
        })
    ...
    return (<MobileShell>
      <div className="relative">
        ...
        {!locationReady && (<div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center text-center px-6 z-[400]">
            ...
            <button onClick={location.turnOn} disabled={location.loading} className="mt-4 inline-flex items-center gap-1.5 h-10 px-5 rounded-full bg-gradient-to-r from-violet-500 to-pink-500 text-white text-[12px] font-bold shadow disabled:opacity-60">
              {location.loading ? (<span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin"/>) : (<LocateFixed className="w-3.5 h-3.5"/>)}
      ...
      <div className="px-5 -mt-6 relative">
        <div className="bg-white rounded-3xl p-5 shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500 flex items-center gap-1">
                ...
                {liveConnected && (<span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">
          ...
          {delivered && !reviewed && (<button onClick={() => setReviewOpen(true)} className="mt-4 w-full h-12 rounded-2xl bg-gradient-to-r from-violet-500 to-pink-500 text-white font-bold shadow-md shadow-pink-200">
              ⭐ Rate your order
            </button>)}

          {reviewed && activeOrder?.items?.[0]?.restaurantId && (<div className="mt-4 p-3 rounded-2xl bg-emerald-50 border border-emerald-200">
              <div className="flex items-center gap-1 mb-1">
                {[1, 2, 3, 4, 5].map((n) => (<Star key={n} className={`w-4 h-4 ${n <= rating ? "fill-amber-400 text-amber-400" : "text-slate-300"}`}/>))}
              ...
              <Link to="/restaurant/$id" params={{ id: activeOrder.items[0].restaurantId }} className="mt-2 inline-block text-xs font-bold text-violet-600">
        ...
        <div className="mt-3 space-y-3">
          {steps.map((s, i) => {
            ...
            return (<div key={s.label} className="flex items-start gap-3">
                <div className={`mt-0.5 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${done
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}>
        <DialogContent className="sm:max-w-[400px] rounded-3xl">
          ...
          <div className="flex justify-center gap-2 my-3">
            {[1, 2, 3, 4, 5].map((n) => (<button key={n} onClick={() => setRating(n)} aria-label={`Rate ${n}`}>
                <Star className={`w-9 h-9 ${n <= rating ? "fill-amber-400 text-amber-400" : "text-slate-300"}`}/>
              </button>))}
          </div>
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Tell us more (optional)" rows={3} className="w-full p-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300"/>
          <button onClick={submitReview} className="mt-2 w-full py-3 rounded-2xl bg-gradient-to-r from-violet-500 to-pink-500 text-white font-bold">
            Submit review
          </button>
        </DialogContent>
      </Dialog>
